...
class NonsaturatingGAN(gan.GAN):
    def set_targets(self,target_dim):
        self.target_g = torch.ones(self.batch_size,target_dim,device=self.device)
        self.target_real = self.label_smooth * torch.ones(self.batch_size,target_dim,device=self.device)
        self.target_fake = torch.zeros(self.batch_size,target_dim,device=self.device)
    ...
    def train_loop(self,images,labels):
        ...
        noise = torch.randn(self.batch_size,self.G.z_size,device=self.device)
        fake = self.G(noise)
